...
def add_trader(session, name, location, repo=None):
    trader = Trader(name=name, location=location, repo=repo)
    session.add(trader)
    return trader
...
def find_repo(session, path, repo):
    query = session.query(Trader).filter(Trader.repo == repo).filter(Trader.location == path)
    return query.all()
